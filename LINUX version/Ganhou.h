int ganhou(int screenwidth, int screenheight, ALLEGRO_DISPLAY *display, ALLEGRO_BITMAP *backgr, ALLEGRO_BITMAP *bomba, score atual)
{
  ...
  //Carregando fontes
	...
	ALLEGRO_FONT *font1 = al_load_ttf_font("fonts/retro3d.ttf", 100, 0);
  ...
  //Carregando eventos
	...
	while(!done)
	{
		...
		al_draw_text(font2, Branco, screenwidth/2 + distancia - 100, screenheight/altura, ALLEGRO_ALIGN_CENTRE, atual.nome);
		al_flip_display();
	}
  //
}
